...
void openPauseMenu(time_t& stageStartTime) {
    ...
    while (inPauseMenu) {
        ...
        if (opt == '1' || opt == 'p' || opt == 'P') {
            ...
            cout << pad << "Press [S] to toggle SFX, [T] to change Text Speed, or any other key to go back.";
            
            char setOpt = _getch();
            if (setOpt == 's' || setOpt == 'S') sfxEnabled = !sfxEnabled;
            if (setOpt == 't' || setOpt == 'T') textSpeed = (textSpeed + 1) % 3; 
    ...
    // Shift the start time forward by the exact amount of time spent paused to freeze the clock
...
int main() {
    ...
    // Vectors for managing the event log and player inventory, allowing for dynamic tracking of recent events and items collected. 
    //The event log provides feedback and narrative context to the player, while the inventory allows for item management and strategic use of resources found throughout the stages.
    ...
    while (state != GameState::EXIT) {
        if (state == GameState::MAIN_MENU) {
            ...
            do { 
                choice = _getch(); 
            } while (choice < '1' || (choice > '6' && choice != 'q' && choice != 'Q'));
            sfxMenuSelect();
            ...
            if (choice == '1') {
                ...
                cout << pad << "Game theme inspiration: Silent Hill, Resident Evil, Darkwood, Utaho no Tatari, Exit 8\n";
        ...
        while (state == GameState::PLAYING) {
            ...
            // This loop ignores accidental misclicks to stop screen flickering!
            while (!validInput) {
                input = _getch(); 
            ...
            if (!isHiding) {
                switch (input) {
                    case 'p': case 'P':
                        openPauseMenu(stageStartTime);
                        continue; // Bypasses the shadow AI loop to safely pause time!
                    case 'd': case 'D':
            ...
            if (validMove || actionTaken) {
                ...
                if (flashlightActive && batteryLevel > 0) {
                    if (isNightmare) batteryLevel -= 2; 
                        ...
                        if (stageMoves % 3 == 0) batteryLevel -= 1; 
                    } 
                    else { 
                        if (stageMoves % 2 == 0) batteryLevel -= 1; 
                    }
                }
                if (batteryLevel <= 0) { 
                    batteryLevel = 0; 
                ...
                if (!onPillar) {
                    if (!flashlightActive || distToShadow <= 5) {
                        if (isNightmare) sanityLevel -= 3; 
                        else if (isBeginner) sanityLevel -= 1; 
                        else sanityLevel -= 2;
                    } 
...
void printTitleScreen() {
    system("cls"); 
    cout << "\033[31m"; // CHANGED: From Cyan (36m) to Blood Red (31m)
    drawDivider('=');
...
void printGameOverScreen() {
    cout << "\033[31;1m"; // CHANGED: Added ;1m to make the Game Over red Bold/Intense
    drawDivider('=');
...
void printVictoryScreen() {
    cout << "\033[31m"; // CHANGED: From Happy Green (32m) to Sinister Red (31m)
    drawDivider('=');
...
bool isSafeZone(int stage, int f, int x) { 
    for(int p=0; p<3; p++) 
    if(pillars[stage][f][p].active && pillars[stage][f][p].pos.x == x) 
    return true; 
    return false; 
}
...
void generateTrapsForStage(int stage) {
    ...
    if (maxFloors[stage] > 0) { 
        ...
        if (terrain3D[stage][maxFloors[stage] - 1][x] == '#') 
        rightmostLadder = x; 
    }
    ...
    for (int y = maxFloors[stage]; y >= 0; y--) {
        ...
        for (int x = 0; x <= stageLengths[stage]; x++) {
            ...
            if (!inVision) { 
                ...
                bool isHole = false; 
                for(int h=0; h<3; h++) if (holes[stage][h].x == x && holes[stage][h].y == y) isHole = true;
                bool isWarp = (warpGates[stage][0].x == x && warpGates[stage][0].y == y) || (warpGates[stage][1].x == x && warpGates[stage][1].y == y);
                bool isLocker = false; 
                for(int l=0; l<4; l++) if (lockers[stage][l].x == x && lockers[stage][l].y == y) isLocker = true;
                bool isFake = false; 
                for(int f=0; f<3; f++) if (fakes[f].active && fakes[f].pos.x == x && fakes[f].pos.y == y) isFake = true;
                bool isPillar = false; 
                for(int p=0; p<3; p++) if (pillars[stage][y][p].active && pillars[stage][y][p].pos.x == x) isPillar = true;
                ...
                if (player.x == x && player.y == y) tile = (isHiding ? "\033[36mP\033[0m" : "\033[32mP\033[0m"); 
                    ...
                    if (s == '+') tile = "\033[32m+\033[0m"; 
                    else if (s == 'b') tile = "\033[36mb\033[0m"; 
                    else if (s == '*') tile = "\033[37m*\033[0m"; 
                    else if (s == '?') tile = "\033[34m?\033[0m";
                    else if (s == 'E') tile = "\033[35mE\033[0m"; 
                    else if (s == '^') tile = "\033[37m^\033[0m";
...
void processShadowAI(const Position& player, Position& shadow, int stage, bool isHiding, Position& soundDistraction, bool hasSight, int& shadowStunTimer, int& shadowFearTimer) {
    auto canMoveTo = [&](int f, int x) { 
        return !isWall(stage, f, x) && !isSafeZone(stage, f, x); 
    };
    ...
    if (shadow.x < stageLengths[stage] && canMoveTo(shadow.y, shadow.x+1)) { 
        if (scentMap[stage][shadow.y][shadow.x+1] > bestScent) { 
            bestScent = scentMap[stage][shadow.y][shadow.x+1]; 
            targetX = shadow.x+1; 
        } 
    }
    if (shadow.x > 0 && canMoveTo(shadow.y, shadow.x-1)) { 
        if (scentMap[stage][shadow.y][shadow.x-1] > bestScent) { 
            bestScent = scentMap[stage][shadow.y][shadow.x-1]; 
            targetX = shadow.x-1; 
            targetY = shadow.y; 
    ...
    if (canGoUp(shadow, stage)) { 
        if (scentMap[stage][shadow.y+1][shadow.x] > bestScent) { 
            bestScent = scentMap[stage][shadow.y+1][shadow.x]; 
            targetX = shadow.x; 
            targetY = shadow.y+1; 
        } 
    }
